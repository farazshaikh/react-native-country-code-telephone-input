...
class PinPicker extends React.Component {
  ...
  updateFocus() {
    if (this.state.setFocusIdx !== undefined)
      this.refs[
        Math.min(this.state.setFocusIdx, this.props.numDigits - 1)
      ].focus();
  }
  ...
  renderPinInput() {
    ...
    for (let i = 0; i < this.props.numDigits; i++) {
      let onChangeText = text => {
        ...
        digits[i] = text;
        this.setState(
          { digits: digits, setFocusIdx: setFocusIdx },
      ...
      let onKeyPress = () => {
        ...
        if (
          ...
          this.setState(
            { releaseFocus: releaseFocus, setFocusIdx: focusIdx },
