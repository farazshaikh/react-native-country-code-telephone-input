...
class PhoneNumberPicker extends React.Component {
  ...
  getCountryFromCCA2(cca2) {
    ...
    if (
      ...
      countryName =
        this.state.country.callingCode.length > 0
    ...
    return { name: countryName, cca2: cca2, callingCode: callingCode };
  }
  ...
  getCountryFromCallingCode(callingCode, phoneNumber) {
    ...
    do {
      ...
      if (
        CallingCodeToCCA2.country_phone_code_to_countries[callingCode] &&
  ...
  CallingCodeChanged(updatedCallingCode) {
    let countryFromCallingCode = this.getCountryFromCallingCode(
      updatedCallingCode,
      this.state.phoneNo,
    );
